            ...
            case 'buy':
                ...
                if (global.db.data.users[m.sender].kucing == 1) return conn.reply(m.chat, 'Kamu sudah memiliki pet ini', m)
                    ...
                    if (global.db.data.users[m.sender].anjing == 1) return conn.reply(m.chat, 'Kamu sudah memiliki pet ini', m)
                        ...
                        break
case 'kyubi':
                    if (global.db.data.users[m.sender].kyubi == 1) return conn.reply(m.chat, 'Kamu sudah memiliki pet ini', m)
                            if (global.DATABASE.data.users[m.sender].emas >= Bkyubi * count) {
                                global.DATABASE.data.users[m.sender].kyubi += count * 1
                                global.DATABASE.data.users[m.sender].emas -= Bkyubi * count
                                conn.reply(m.chat, `✔️ Sukses Membeli Pet kyubi 🦊 Dengan Harga ${Bkyubi * count} Gold 🪙`, m)
                            } else conn.reply(m.chat, `Gold Anda Tidak Cukup`, m)
                ...
                switch (_type) {
                	case 'fishingrod':
                        ...
                        break
                            ...
                            break
            default:
                return await conn.sendButton(m.chat, Kchat, wm, null, [['INVENTORY', '#inv']], m)
