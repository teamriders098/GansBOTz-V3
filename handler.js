                    ...
                    for (let user of participants) {
                       let pp = './src/avatar_contact.png'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Yah,si Beban Masuk Grup').replace('@user', '@' + user.split('@')[0]).replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc) :
                                (chat.sBye || this.bye || conn.bye || 'Sip, Beban Berkurang 1').replace('@user', '@' + user.split('@')[0]).replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc))
                                this.sendButtonImg(id, pp, text, "Group Message By GansBOTz", "\nGw Pedo ygy", ".wkwk", null)
                                }
                    }
                }
                break
